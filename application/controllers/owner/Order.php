...
class Order extends CI_Controller {
	 ...
	 * Get pending orders by room id - getPendingOrdersByRoomID()
...
<th>Recipe Details</th>
	...
	foreach ($order['items'] as $key => $item) {
        ...
        '</td>
					...
					<td style="width:80px;"><input type="text" class="form-control rate" disabled style="width: 100%;" value="' . $item['rate'] . '"></td>
					<td class="amount">' . $item['rate'] * $item['quantity'] . '</td>
					<td class="total-amount">' . $item['total_amount'] . '</td>
					<td><input type="checkbox" class="form-check-input" disabled name="is_addon" value="1" ' . ($item['is_addon'] == 1 ? 'checked' : '') . '></td>
                    <td style="width:150px;">' . $item['item_remarks'] . '</td>
		...
		public function getProductRatesWithIsCustomizeNewDiningOrder(){
            ...
            <select class="form-select" name="variant_id" id="variant_id">'; // Default placeholder option
				foreach ($variantsList as $variant1) {
    				$html .= '<option data-variant_value="' . $variant1['variant_value'] . '" value="' . htmlspecialchars($variant1['variant_id']) . '">' . htmlspecialchars($variant1['variant_name']) . '</option>';
				}
				$html .= '</select>
        	</div>
			<div class="row">
...
$html .= '<div class="col">
    ...
    <select class="form-select" name="variant_id" id="variant_id">'; // Default placeholder option
        foreach ($variantsList as $variant1) {
        $html .= '<option data-variant_value="' . $variant1['variant_value'] . '" value="' . htmlspecialchars($variant1['variant_id']) . '">' .
